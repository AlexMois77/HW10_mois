        ...
        with open(json_file, 'r', encoding='utf-8') as file:
            ...
            for item in data:
                ...
                if not Quote.objects.filter(author=author, text=quote_text).exists():
                    ...
                    self.stdout.write(self.style.SUCCESS(f'Successfully added quote: "{quote_text}" by {author.name}'))
                else:
                    self.stdout.write(self.style.WARNING(f'Quote already exists: "{quote_text}" by {author.name}'))
